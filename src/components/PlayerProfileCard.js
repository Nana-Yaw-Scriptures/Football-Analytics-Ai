...
═══════════════════════════════════════════ */
const POS_CONFIG = {
  Forward:    { color: '#ef4444', bg: 'rgba(239,68,68,0.12)',   border: 'rgba(239,68,68,0.3)',   short: 'FWD', grad: 'from-red-500/20 to-transparent'     },
  Attacker:   { color: '#ef4444', bg: 'rgba(239,68,68,0.12)',   border: 'rgba(239,68,68,0.3)',   short: 'ATT', grad: 'from-red-500/20 to-transparent'     },
  Midfielder: { color: '#60a5fa', bg: 'rgba(96,165,250,0.12)',  border: 'rgba(96,165,250,0.3)',  short: 'MID', grad: 'from-blue-500/20 to-transparent'    },
  Defender:   { color: '#34d399', bg: 'rgba(52,211,153,0.12)',  border: 'rgba(52,211,153,0.3)',  short: 'DEF', grad: 'from-emerald-500/20 to-transparent'  },
  Goalkeeper: { color: '#fbbf24', bg: 'rgba(251,191,36,0.12)',  border: 'rgba(251,191,36,0.3)',  short: 'GK',  grad: 'from-yellow-500/20 to-transparent'   },
};
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      ...
      {[0.25, 0.5, 0.75, 1].map((f, i) => (
        <polygon key={i} points={grid(f)} fill={f === 1 ? `${color}06` : 'none'} stroke="rgba(255,255,255,0.06)" strokeWidth="0.8"/>
      ))}
      {/* Spokes */}
      {dims.map((_, i) => {
        const p = pt(i, 1 / Math.min(vals[i] || 0.01, 1));
        const end = { x: cx + Math.cos((Math.PI * 2 * i) / n - Math.PI / 2) * r, y: cy + Math.sin((Math.PI * 2 * i) / n - Math.PI / 2) * r };
        return <line key={i} x1={cx} y1={cy} x2={end.x} y2={end.y} stroke="rgba(255,255,255,0.05)" strokeWidth="0.7"/>;
      })}
      {/* Fill */}
      <path d={path} fill={`${color}20`} stroke={color} strokeWidth="2"
      ...
      {dims.map((d, i) => {
        ...
        return (
          <text key={i} x={lx} y={ly} textAnchor="middle" dominantBaseline="middle"
            fill="rgba(255,255,255,0.45)" fontSize="9.5" fontWeight="600" fontFamily="Outfit">
            {d.label}
          </text>
        );
      })}
...
const StatRow = ({ label, value, color, suffix = '', decimal = false, duelsFmt = false, player }) => {
  ...
  return (
    <div className="flex items-center justify-between py-2 border-b border-white/[0.04] last:border-0">
      ...
      <span className="text-[13px] font-black" style={{ fontFamily: 'JetBrains Mono', color: color }}>{display}</span>
    </div>
  );
};
...
const XGIntel = ({ player }) => {
  ...
  if (absD < 0.5) return null;
  const over   = diff > 0;
  const color  = over ? '#10b981' : '#ef4444';
  const label  = over
    ? absD > 3 ? 'Elite Finisher' : 'Overperforming xG'
    : absD > 3 ? 'Wasteful'       : 'Underperforming xG';
  return (
    <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl" style={{ background: `${color}12`, border: `1px solid ${color}25` }}>
      ...
      <span className="text-[11px] font-black ml-auto" style={{ fontFamily: 'JetBrains Mono', color }}>
  ...
  return (
    <div className="rounded-2xl border border-white/[0.07] overflow-hidden" style={{ background: 'rgba(10,14,26,0.8)' }}>
      ...
      <div className="px-4 pb-1">
        {rows.map((r, i) => (
          <StatRow key={i} label={r.label} value={r.value} color={r.color}/>
        ))}
      </div>
    </div>
  );
...
export default function PlayerProfileCard({ player, onClose }) {
  ...
  const ratingColor = rating >= 7.5 ? '#10b981' : rating >= 7 ? '#22d3ee' : rating >= 6.5 ? '#f59e0b' : '#6b7280';
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4"
      ...
      <div className="w-full sm:max-w-md rounded-t-3xl sm:rounded-3xl overflow-hidden shadow-2xl"
        ...
        <div className="relative overflow-hidden">
          {/* Background gradient — position-coloured */}
          <div className="absolute inset-0" style={{
            background: `linear-gradient(160deg,${posC.color}18 0%,rgba(6,10,20,0.95) 55%)`,
          }}/>
          ...
          <div className="relative flex items-center justify-between px-5 pt-4 pb-2">
            <div className="flex items-center gap-2">
              ...
              {isHot && (
                <span className="text-[10px] font-black px-2 py-0.5 rounded-full" style={{ background: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.3)', color: '#ef4444' }}>
                  🔥 Hot Form
                </span>
              )}
            </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0 pt-1">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {/* Nationality flag */}
                  {player.nationality && (
                    <span className="text-[12px] text-slate-300 font-semibold">{player.nationality}</span>
                  )}
                  {player.age > 0 && (
                    <span className="text-[12px] text-slate-500">{player.age}y</span>
                  )}
                </div>
                {/* Physical */}
        ...
        <div className="flex border-b border-white/[0.07]" style={{ background: 'rgba(0,0,0,0.2)' }}>
          {[
            ...
            <button key={t.id} onClick={() => setTab(t.id)}
              ...
              {tab === t.id && (
                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 h-0.5 w-8 rounded-full" style={{ background: posC.color }}/>
              )}
        ...
        <div className="p-4 space-y-3" style={{ animation: 'fadeIn 0.2s ease-out' }}>
              ...
              {stats.sections.map((section, si) => (
                <div key={si} className="rounded-2xl border border-white/[0.07] overflow-hidden" style={{ background: 'rgba(10,14,26,0.8)' }}>
                  <div className="px-4 py-3 border-b border-white/[0.06] flex items-center gap-2"
                    style={{ background: `${section.color}07` }}>
                        ...
                        <div className="flex items-center justify-between mb-1.5">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-[12px] font-black" style={{ fontFamily: 'JetBrains Mono', color: gc }}>
                              {(parseFloat(player[dim.key]) || 0).toFixed(dim.key === 'rating' || dim.key === 'xG' || dim.key === 'xA' || dim.key === 'duelWinPct' || dim.key === 'shotAccuracy' || dim.key === 'dribbleSuccessPct' ? 1 : 0)}
                            </span>
                          </div>
                        </div>
          ...
          {/* PER 90 TAB */}
          {tab === 'per90' && (
            <Per90Section player={player} posGroup={pos}/>
          )}
        </div>
